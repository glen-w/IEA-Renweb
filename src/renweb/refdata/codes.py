...
@cache
def growth_parameters() -> dict[str, float]:
    raw = json.loads(read_text("growth_parameters.json"))
    return {str(key): float(value) for key, value in raw.items()}


def district_heat_bioenergy_share() -> float:
    return float(growth_parameters().get("district_heating_bioenergy_share", 0.95))
