...
def compute_one(
    ...
    sigma_industry = elec_for_heat_share(country, "industry", year)
    sigma_buildings = elec_for_heat_share(country, "buildings", year)
    _add(out, "assumption_elec_for_heat_industry", sigma_industry, SHARE)
    _add(out, "assumption_elec_for_heat_buildings", sigma_buildings, SHARE)
    _add(out, "assumption_bioenergy_share", bioenergy_share(country), SHARE)
    ...
    def heat_demand(flows: tuple[str, ...]) -> float:
        total = 0.0
        for flow in flows:
            bio = max(_bioenergy(frame, flow), 0.0)
            total += (
                frame.get(flow, "HEAT")
                + frame.get(flow, "SOLARTH")
                + frame.get(flow, "GEOTHERM")
    ...
    share_for_reh = final_renew_share or 0.0
    reh_industry = heat_demand(("TOTIND",)) * sigma_industry * share_for_reh
    reh_buildings = (
        heat_demand(("RESIDENT", "COMMPUB")) * sigma_buildings * share_for_reh
    )
    reh_agri = heat_demand(("AGRICULT", "FISHING")) * sigma_industry * share_for_reh
    _add(out, "renewable_electricity_heat_industry_tj", reh_industry, ENERGY)
    _add(out, "renewable_electricity_heat_buildings_tj", reh_buildings, ENERGY)
    _add(out, "renewable_electricity_heat_agriculture_tj", reh_agri, ENERGY)
    _add(
        out,
        "renewable_electricity_heat_total_tj",
        reh_industry + reh_buildings + reh_agri,
        ENERGY,
    )
    ...
    if share_for_reh:
        elec_for_heat = (reh_industry + reh_buildings + reh_agri) / share_for_reh
    else:
        elec_for_heat = 0.0
    transport_elec = frame.get("TOTTRANS", "ELECTR")
    _add(out, "sectoral_electricity_for_heat_tj", elec_for_heat, ENERGY)
    _add(
        out,
        "sectoral_electricity_excluding_heat_and_transport_tj",
        tfc_elec - transport_elec - elec_for_heat,
        ENERGY,
    )
    ...
    bio_district = district_heat_bioenergy_share()
